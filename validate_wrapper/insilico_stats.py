...
def coverage_stats(coverage, thresholds):
    with open(coverage, 'r') as coverage_file:
        ...
        coverage_stats["mean"] = str(calculate_mean(coverage_list))
        coverage_stats["stdev"] = str(calculate_stdev(coverage_list))

        for threshold in threshold_list:
            coverage_stats[f"%>={threshold}X"] = str(count_bases_in_threshold(coverage_list, int(threshold)))

        data = []
        header = []
        for coverage_stat in coverage_stats:
            header.append(coverage_stat)
            data.append(coverage_stats[coverage_stat])
...
#    parser.add_argument('-o', '--output', nargs='?', help='(Output location)', required=True)
    ...
    coverage_stats(args.coverage, args.thresholds)
